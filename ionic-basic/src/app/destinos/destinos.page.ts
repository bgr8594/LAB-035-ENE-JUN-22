...
export class DestinosPage implements OnInit {
  ...
  editarLugar(id: any, lugar: any) {
    this.editando = true;
    this.lugar = lugar;
    this.estado = "Editar el lugar de destino";
  ...
  getPosition(): Promise<any> {
    return new Promise((resolve: any, reject: any): any => {
      ...
      (err: any) => {
        if ( err.code === 1 ) {
          alert('Activar geolocalizaciòn en el navegador.');
        }
        this.latitud = null;
        this.longitud = null;
  ...
  async addDirection(){
    ...
    if(data){
      ...
      console.log('Ubicacion actualizada, latitud: '+this.latitud+' \nlongitud:'+this.longitud);
    }
  } 
}